...
def residual_combine(
    ...
    P=False,
    max_regular_groups=None,
    max_residual_groups_per_lane=4,
    parallel_groups=4,
    reuse_depth=None,
    new_residual_group_penalty=1,
    candidate_search_limit=None,
):
    ...
    if max_conflict is not None and max_conflict < 0:
        raise ValueError("max_conflict must be greater than or equal to 0, or None.")

    if max_regular_groups is None:
        # Old default was max_regular_groups = mux_size.
        max_regular_groups = mux_size
    if max_regular_groups <= 0:
        raise ValueError("max_regular_groups must be greater than 0.")

    if reuse_depth is None:
        reuse_depth = math.ceil(int(max_regular_groups) / int(parallel_groups))
    reuse_depth = max(1, int(reuse_depth))

    result = rcc.residual_combine(
        matrix=matrix,
        mux_size=mux_size,
    ...
    return result.packed, result.group_len, result.gidx
...
def ctf(now_t, now_col_in_grps, next_t, mux_size, sa_size = 16, ope = True):
